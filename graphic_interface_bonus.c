...
/*   Updated: 2024/06/26 19:15:59 by dsatge           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	step_status(t_game *game)
{
	...
	img_and_transp(game, "sprites/extra/sheet1.xpm", game->tab_y_len,
		game->tab_x_len - 2);
	step_num = ft_itoa(game->step_count);
	step_char = ft_strjoin("Step count : ", step_num);
	mlx_string_put(game->mlx, game->window, (game->tab_x_len - 1.2) * 64,
		(game->tab_y_len + 0.5) * 64, 0x7e7c78, step_char);
	free(step_num);
	free(step_char);
	if (game->finish != 1)
		game->step_count++;
